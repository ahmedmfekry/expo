...
export default function ConsumeStock({ navigation, route }){
  ...
  async function save(){
    ...
    try{
      ...
      await db.createDocument('[DATABASE_ID]', 'consumes', undefined, payload);
      Alert.alert('تم الصرف');
      navigation.goBack();
    }catch(e){
  ...
  function onChangeDate(event, selected){
    ...
    if (selected) {
      // set both date and expire as sample (you can customize)
      // date is managed as const earlier; for now we'll set expire only
      setExpire(selected);
    }
  }
  ...
  return (
    <View style={styles.container}>
      <Card style={{padding:12}}>
        <Text variant="titleMedium" style={styles.label}>تاريخ الصرف</Text>
